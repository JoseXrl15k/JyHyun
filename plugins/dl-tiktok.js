...
const handler = async (m, {conn, text, args, usedPrefix, command}) => {
 ...
 await m.react('🕒') 
    ...
    const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: texto, contextInfo: {externalAdReply: {title: packname, body: wm, thumbnail: imagen0, sourceUrl: canal}, mentionedJid: [m.sender]}}}, aa);
...
async function tiktokdlF(url) {
  if (!/tiktok/.test(url)) return conn.reply(m.chat, `🚩 Te faltó el link del video de tiktok.`, m);
