...
const groq = new Groq({ apiKey: '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' });
    ...
    if (!ya) throw `Por favor ingresa un texto. Ejemplo: ${usedPrefix + command} Hola`
      try {
        ...
        conn.sylph[m.sender] = [{
          role: 'system',
          content: `Eres Jy Hyun, una inteligencia artificial creada por Jose Elber, responde de manera clara y concisa para que los usuarios entiendan mejor tus respuestas. El nombre del usuario será: ${conn.getName(m.sender)}`,
        }];
...
handler.command = ['ai2', 'openai2', 'ia2', 'chatgpt2'];
handler.help = ['openai2', 'ia2', 'chatgpt2'];
handler.tags = ['tools'];
