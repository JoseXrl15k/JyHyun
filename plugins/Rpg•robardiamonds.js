...
return;
  ...
  if (users.diamond < rob) return conn.reply(m.chat, `😔 @${who.split`@`[0]} Tiene menos de *${ro} diamond 💎*\nNo robes a un pobre :v`, m, {mentions: [who]});
  global.db.data.users[m.sender].diamond += rob;
  global.db.data.users[who].diamond -= rob;
  conn.reply(m.chat, `*🚩 Robastes ${rob} diamond 💎 a @${who.split`@`[0]}*`, m, {mentions: [who]});
