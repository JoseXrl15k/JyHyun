...
if (!text) return m.reply(`*🍟 Ingrese el nombre de la canción*\n\n*Ejemplo:*\n#play phonk`) 
...
await conn.sendFile(m.chat, dl_url, ttl + '.mp3', null, m, { mimetype: 'audio/mp4' })
} catch (e2) {
try {   
const downloadUrl = await fetch9Convert(yt_play[0].url); 
await conn.sendFile(m.chat, downloadUrl, 'audio.mp3', null, m, { mimetype: 'audio/mp4' })
} catch (e3) {
try {
...
_*Descargado sus video, aguarden un momento....*_`.trim();
...
if (command == 'play001' || command == 'playdoc001') {
if (!text) return m.reply(`*Que está buscando? 🍟*\n*Ingrese el nombre de la canción*\n\n*Ejemplo:*\n#play phonk`) 
const yt_play = await search(args.join(' '));
const ytplay2 = await yts(text);
const texto1 = `🍟 *Título* : ${yt_play[0].title}
...
_*Descargado el audio 🚩 en documentos, aguarden un momento por favor....*_`.trim();
...
if (command == 'play003' || command == 'playdoc002') {
if (!text) return m.reply(`*Que está buscando? 🍟*\n*Ingrese el nombre de la canción*\n\n*Ejemplo:*\n#play phonk`) 
const yt_play = await search(args.join(' '));
const ytplay2 = await yts(text);
const texto1 = `🍟 *Título* : ${yt_play[0].title}\n🚩 *Publicado:* ${yt_play[0].ago}\n🌸 *Duración:* ${secondString(yt_play[0].duration.seconds)}

*Descargado sus video en documentos, aguarden un momento por favor....*`.trim();
...
handler.help = ['play', 'play2', 'play003', 'play004', 'playdoc001'];
handler.tags = ['downloader'];
handler.command = ['play', 'play2', 'play003', 'play004', 'audio', 'video', 'playdoc001', 'playdoc002']
//handler.limit = 3
